class SmsSettingController {
  constructor(smssettngUseCase) {
    this.smssettngUseCase = smssettngUseCase;
  }
  ...
  async addSmsSetting(req, res) {
    try {
      ...
      const response = await this.smssettngUseCase.addSmsSetting(req.body);
  ...
  async getByBranch(req, res) {
    try {
      ...
      const response = await this.smssettngUseCase.getSettingByBranch(branchId);
  ...
  async getByProjectAndBranch(req, res) {
    try {
      ...
      const response = await this.smssettngUseCase.getByProjectAndBranch(
        projectId,
        branchId
      );
      return res.status(response.success ? 200 : 404).json(response);
